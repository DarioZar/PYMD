...
class NVEState:
    ...
    def corrections(self, rc: float, rho: float,
                    use_e_corr: bool) -> Tuple[float, float, float]:
        # Compute the tail-corrections; assumes sigma and epsilon are both 1
        rr3 = 1 / rc**3
        ecor = (8 * np.pi * rho *
                ((rr3**3) / 9 - rr3 / 3) if use_e_corr else 0)
        pcor = (16 / 3 * np.pi * (rho**2) *
                (2 / 3 * (rr3**3) - rr3) if use_e_corr else 0)
        ecut = 4 * (rr3**4 - rr3**2)
    ...
    def calc_vars(self, vir: float):
        ...
        self.P = (self.atoms.rho * self.KE * 2.0 / 3.0 / self.atoms.N +
