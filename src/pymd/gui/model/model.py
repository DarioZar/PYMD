...
from pymd.state import NVEState, NVTAndersenState, available_statistics
...
class Model:
    def __init__(self):
        ...
        self.values = dict.fromkeys({
            'N', 'elem', 'rho', 't0', 'rc', 'steps', 'dt', 'fSamp',
            'use_ecorr', 'append', 'unfold', 'outputfile', 'Tbath', 'nu'
        })
        self.element = None
        self.atoms = None
        self.state = None
    ...
    def setSimulationData(self):
        ...
        if self.fileName:
            ...
            self.atoms = Atoms(
                self.values['N'], self.values['rho'], self.element)
        if self.mode == 0:
